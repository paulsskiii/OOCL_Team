...
import { Truck, Container as LucideContainer, Ship, PlusCircle, Search } from 'lucide-react';
...
const App = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Reset the form
      setNewContainer({ containerName: '', origin: '', destination: '', weight: '' });
    } catch (e) {
  ...
  if (loading) return <div className="text-center mt-8 text-2xl font-bold text-gray-700 animate-pulse">Loading containers...</div>;
  if (error) return <div className="text-center mt-8 text-2xl text-red-600 font-bold">{error}</div>;
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-1 bg-white p-6 rounded-3xl shadow-2xl border-2 border-blue-200">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-600">Container Name</label>
                <input 
                  type="text" 
                  name="containerName"
          ...
          <div className="md:col-span-2 bg-white p-6 rounded-3xl shadow-2xl border-2 border-indigo-200">
            ...
            {filteredContainers.length === 0 ? (
              <p className="text-gray-500 italic text-center">No containers match your search or have been added yet.</p>
            ) : (
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center">
                      <div className="flex-1">
                        <p className="text-lg font-bold text-indigo-800">
                          Container ID: <span className="font-normal text-gray-700">{container.containerName}</span>
                        </p>
                        <p className="text-gray-600 text-sm mt-1">
                          From <span className="font-medium text-indigo-600">{container.origin}</span> to <span className="font-medium text-indigo-600">{container.destination}</span>
                        </p>
                        <p className="text-gray-600 text-sm">
                          Weight: <span className="font-medium">{container.weight} kg</span>
                        </p>
                      </div>
