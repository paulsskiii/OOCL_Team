    ...
    public static void main(String[] args) { 
        ...
        try (KafkaProducer<String, String> producer = new KafkaProducer<>(props)) { 
            for (int i = 0; i < 5; i++) { 
                ...
                ProducerRecord<String, String> record2 = new ProducerRecord<>("cargo-events", cargoId2, message); 
                producer.send(record, (metadata, exception) -> { 
                    if (exception == null) { 
                        System.out.printf(cargoId+"Sent record to topic %s, partition %d, offset %d%n", 
                ...
                producer.send(record2, (metadata, exception) -> { 
                    if (exception == null) { 
                        System.out.printf(cargoId2+" Sent record to topic %s, partition %d, offset %d%n", 
                                metadata.topic(), metadata.partition(), metadata.offset()); 
                    } else { 
                        exception.printStackTrace(); 
                    } 
