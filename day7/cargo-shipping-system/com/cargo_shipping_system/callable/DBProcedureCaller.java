...
public class DBProcedureCaller {
    ...
    public void getCustomerDeliveryHistory(String customer_id) {
        List<String> listOfPackageDetails = new ArrayList<>();

        Integer totalSentPackages = null;
        Integer totalReceivedPackages = null;
        ...
        try {
            ...
            while(hadResults) {
                ...
                while (resultSet.next()) {
                    listOfPackageDetails.clear();

                    String customerName = resultSet.getString("customer_name");
                    totalSentPackages = resultSet.getInt("total_sent_packages");
                    totalReceivedPackages = resultSet.getInt("total_recieved_packages");
                    int packageId = resultSet.getInt("package_id");

                    listOfPackageDetails.add(resultSet.getTimestamp("pickup_date").toString());
                    listOfPackageDetails.add(resultSet.getTimestamp("delivery_date").toString());

                    System.out.println("\n============ CUSTOMER DELIVERY HISTORY ==============\n");
                    System.out.println("Customer Name: " + customerName + "\nPackage ID: " + packageId);
                    System.out.println("Pickup Date and Delivery Date: ");
                    for (String string : listOfPackageDetails) {
                        System.out.println(string);
                    }
                }
                System.out.println("\nTotal sent packages: " + totalSentPackages + 
                    "\nTotal received packages: " + totalReceivedPackages);
                hadResults = callableStatement.getMoreResults();
            }
