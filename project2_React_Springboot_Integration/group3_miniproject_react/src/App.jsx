...
function App() {
  ...
  const [invalidId, setInvalidId] = useState(0);
  ...
  function fetchCustomerCount() {
    getCustomerCount()
      .then((response) => {
        ...
        alert("There are " + response.data.count + " customers");
      })
      .catch((error) => {
        console.error(error);
      });
  ...
  return (
    <div className="h-screen lg:h-full w-screen  items-center justify-center font-display">
      ...
      {isInvalidId ? (
        <ErrorMessage
          message={`Invalid Customer ID ${invalidId}. Please input a valid ID.`}
        ></ErrorMessage>
      ) : (
        <></>
      )}
      ...
      <section className="flex flex-row gap-3 items-center justify-center my-10">
        <Button text={"SHOW ALL"} onClick={handleAllButtonClick} />
        <Button text={"GET COUNT"} onClick={handleCountButtonClick} />
